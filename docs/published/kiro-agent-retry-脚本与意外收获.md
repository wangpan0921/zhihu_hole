# 不用再让 AI 等你了：给 kiro-cli 套了个自动重试脚本，顺手治好"按 y 按到手抽筋"的毛病

> 📦 脚本完整源码 / 在线预览 / 一键下载 / 一键复制：
> **<https://ai.lovestoblog.com/apps/kiro-retry/>**
> （这是我自己搭的一个"Agent 工坊"个人站，纯静态、免费托管，不用注册不用登录，直接点进去就能拿。）
...
> 懒人入口：直接打开 **<https://ai.lovestoblog.com/apps/kiro-retry/>** ，左上角"下载脚本"拿到 `kiro-agent-retry.sh`，丢到自己项目的 `scripts/` 目录、`chmod +x` 就能用；右上角"复制全部"也能直接把内容贴进编辑器；下面还有逐行注释的预览，方便先扫一遍再决定要不要装。
